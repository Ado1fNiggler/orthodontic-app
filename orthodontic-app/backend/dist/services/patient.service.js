...
class PatientService {
    ...
    static async createPatient(data) {
        try {
            ...
            if (data.email || data.phone) {
                const existingPatient = await database_js_1.prisma.patient.findFirst({
                    where: {
                        OR: [
                ...
                if (existingPatient) {
                    if (existingPatient.email === data.email?.toLowerCase()) {
                        throw new error_handler_js_1.ConflictError('Patient with this email already exists');
                    }
                    if (existingPatient.phone === data.phone) {
                        throw new error_handler_js_1.ConflictError('Patient with this phone number already exists');
                    }
                }
            }
            // Create patient
            const patient = await database_js_1.prisma.patient.create({
                data: {
                    ...data,
            ...
            logger_js_1.logger.error('Create patient failed', {
    ...
    static async getPatientById(id) {
        try {
            ...
            const [lastAppointment, nextAppointment] = await Promise.all([
                database_js_1.prisma.appointment.findFirst({
                    where: {
                        patientId: id,
                        status: 'COMPLETED',
                    },
            ...
            logger_js_1.logger.error('Get patient by ID failed', {
                patientId: id,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
    ...
    static async updatePatient(id, data) {
        try {
            ...
            const existingPatient = await database_js_1.prisma.patient.findUnique({
                where: { id },
            ...
            if (data.email || data.phone) {
                const conflictingPatient = await database_js_1.prisma.patient.findFirst({
                    where: {
                        OR: [
                            data.email ? { email: data.email.toLowerCase() } : {},
                            data.phone ? { phone: data.phone } : {},
                ...
                if (conflictingPatient) {
                    if (conflictingPatient.email === data.email?.toLowerCase()) {
                        throw new error_handler_js_1.ConflictError('Another patient with this email already exists');
                    }
                    if (conflictingPatient.phone === data.phone) {
                        throw new error_handler_js_1.ConflictError('Another patient with this phone number already exists');
                    }
                }
            }
            // Update patient
            const updatedPatient = await database_js_1.prisma.patient.update({
                where: { id },
                data: {
                    ...data,
                    email: data.email?.toLowerCase(),
            ...
            logger_js_1.logger.error('Update patient failed', {
                patientId: id,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
    ...
    static async searchPatients(params) {
        try {
            const { query = '', page = 1, limit = 20, sortBy = 'lastName', sortOrder = 'asc', isActive = true, } = params;
            ...
            const [patients, total] = await Promise.all([
                ...
                database_js_1.prisma.patient.count({ where }),
            ]);
            return {
                patients,
                pagination: {
            ...
            logger_js_1.logger.error('Search patients failed', {
                params,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
    ...
    static async deactivatePatient(id) {
        try {
            const patient = await database_js_1.prisma.patient.update({
                where: { id },
                data: { isActive: false },
                select: {
                    id: true,
            ...
            logger_js_1.logger.error('Deactivate patient failed', {
                patientId: id,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
     ...
     * Reactivate patient
     */
    static async reactivatePatient(id) {
        try {
            const patient = await database_js_1.prisma.patient.update({
                where: { id },
                data: { isActive: true },
            ...
            logger_js_1.logger.error('Reactivate patient failed', {
                patientId: id,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
     ...
     */
    static async importFromBookingSystem(bookingId, createdBy) {
        try {
            const mysql = await (0, database_js_1.connectMySQL)();
            ...
            logger_js_1.logger.error('Import patient from booking system failed', {
                bookingId,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
     ...
     */
    static async linkAppointmentToPatient(patientId, bookingData) {
        try {
            ...
            return await database_js_1.prisma.appointment.create({
                data: appointmentData,
            });
        }
        catch (error) {
            logger_js_1.logger.error('Link appointment to patient failed', {
                patientId,
    ...
    static async getPatientStats() {
        try {
            const [totalPatients, activePatients, newPatientsThisMonth, patientsWithTreatmentPlans, patientsWithAppointments,] = await Promise.all([
                database_js_1.prisma.patient.count(),
                database_js_1.prisma.patient.count({ where: { isActive: true } }),
                database_js_1.prisma.patient.count({
                    where: {
                        createdAt: {
            ...
            logger_js_1.logger.error('Get patient stats failed', {
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
        }
    ...
    static async getRecentPatients(limit = 10) {
        try {
            return await database_js_1.prisma.patient.findMany({
                where: { isActive: true },
                select: {
                    id: true,
                    firstName: true,
            ...
            logger_js_1.logger.error('Get recent patients failed', {
                error: error instanceof Error ? error.message : 'Unknown error',
            });
            throw error;
        }
